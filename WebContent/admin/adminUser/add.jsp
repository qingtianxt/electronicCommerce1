	...
	<div class="row-fluid" style="margin-top: 200px;">
		...
		<div class="col-md-6">
			<form role="form" class="form-horizontal"
				action="${pageContext.request.contextPath}/adminUser?method=add&updateId=${updateBean.id }"
				method="post" id="checkForm">
				<div class="form-group">
					<label class="col-md-3 control-label" for="username">用户名</label>
					<div class="col-md-9">
						<c:if test="${empty updateBean }">
							<input class="form-control" name="username" type="text" id="username" placeholder="Username" />
						</c:if>
						<c:if test="${not empty updateBean }">
							<input class="form-control" name="username" type="text" id="username" placeholder="Username" value="${updateBean.username }" readonly="readonly" />
						</c:if>
					</div>
				</div>
				<div class="form-group">
